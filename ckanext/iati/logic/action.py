...
def _user_list(context, data_dict):
    ...
    sort_order = data_dict.get('sort_order', 'asc')  # Get sort order
    ...
    # Determine if we need descending order
    sort_direction = _desc if sort_order == 'desc' else _asc

    order_by_field = None
    if order_by == 'number_created_packages':
        order_by_field = model.User.number_created_packages
    ...
    if order_by == 'display_name' or order_by_field is None:
        ...
        query = query.order_by(sort_direction(order_by_field), sort_direction(model.User.name))
...
def user_list(context, data_dict):
    ...
    return _user_list(context, data_dict)
